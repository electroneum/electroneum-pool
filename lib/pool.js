...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            ...
            var noRetarget = portData.noRetarget;
            if(config.poolServer.fixedDiff.enabled) {
                ...
                if(fixedDiffCharPos != -1) {
                    noRetarget = true;
                    difficulty = parseInt(login.substr(fixedDiffCharPos + 1)) || portData.difficulty;
                    if(!difficulty || difficulty < config.poolServer.varDiff.minDiff) {
                        difficulty = config.poolServer.varDiff.minDiff;
                        noRetarget = portData.noRetarget;
                    }
